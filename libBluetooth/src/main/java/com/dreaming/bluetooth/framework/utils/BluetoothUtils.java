...
import com.dreaming.bluetooth.framework.R;
...
public class BluetoothUtils {
    ...
    private static BluetoothLogger logger = new BluetoothLogger(BluetoothUtils.class);
    ...
    private static BluetoothLogger.Tag tagOfRegisterGlobalReceiver = new BluetoothLogger.Tag("registerGlobalReceiver");
    @SuppressLint("WrongConstant")
    private static void registerGlobalReceiver(BroadcastReceiver receiver, IntentFilter filter) {
        try {
            logger.e(tagOfRegisterGlobalReceiver, "Build.VERSION.SDK_INT:" + Build.VERSION.SDK_INT);
            if (Build.VERSION.SDK_INT >= 33)
                getContext().registerReceiver(receiver, filter, RECEIVER_EXPORTED);
            else
                getContext().registerReceiver(receiver, filter);
        } catch (Exception e) {
            logger.e(tagOfRegisterGlobalReceiver, e);
            getContext().registerReceiver(receiver, filter);
        }
    }
    ...
    @TargetApi(Build.VERSION_CODES.JELLY_BEAN_MR2)
    public static BluetoothDeviceState getConnectStatus(String mac) {
        BluetoothManager manager = getBluetoothManager();
        if (manager != null) {
            try {
                BluetoothDevice device = getRemoteDevice(mac);
                return BluetoothDeviceState.parse(manager.getConnectionState(device, BluetoothProfile.GATT));
            } catch (Throwable e) {
                logger.e(e);
            }
        }
        return BluetoothDeviceState.Unknown;
    }
    ...
    public static boolean refreshGattCache(BluetoothGatt gatt) {
        ...
        logger.v("refreshDeviceCache return %b", result);

        return result;
    }

    public static void printBanner(){
        InputStream is = BluetoothContext.get().getResources().openRawResource(R.raw.banner);
        ...
        try {
            ...
            while (true) {
                ...
                if(line == null) break;
                Log.d("Bluetooth-Kit", line);
            }
            Log.d("", "");
            Log.d("", "");
            Log.d("Version:", "");
            Log.d("Github:", "https://github.com/isong0623/Android-BluetoothKit");
            reader.close();
        } catch (Exception e) {
            e.printStackTrace();
        } finally {
